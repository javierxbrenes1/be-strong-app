import { Genre, Muscle } from '../../../common/enums';
...
const maleMuscleTable: TableType = [
  {
    ...
    ranges: {
      ...
      [Muscle.standard as string]: (value: number) =>
        value >= 44 && value <= 57,
...
export const getMuscleResult = (
  genre: Genre,
  age: number,
  value: number
): Muscle => {
  const table = genre === Genre.male ? maleMuscleTable : femaleMuscleTable;
  ...
  const muscleResult =
    Object.keys(el.ranges).find((k) => el.ranges[k](value)) || Muscle.undefined;
